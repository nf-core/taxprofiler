...
nf-core/taxprofiler currently offers host-removal via minimizer sketcing with Deacon for both short and long reads. For short reads, host removal can alternatively be performed via alignment against a reference genome using Bowtie2 or with Hostile that is using pre-defined indices. For long reads, hostile or minimap2 are offered as alternatives to Deacon. The filtered reads are then used for downstream classification/profiling.
