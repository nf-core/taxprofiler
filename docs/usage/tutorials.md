...
To build a KMCP database you need three components: the FASTA files you wish to include in gzip-compressed format and one genome per file with the reference identifier in the file name, the taxid mapping file, and NCBI taxonomy dump files (names.dmp, nodes.dmp)
